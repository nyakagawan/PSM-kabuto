...
namespace kabuto
{
	...
	public class DebugString  : IDisposable
	{
		...
		GraphicsContext graphics { get; set; }
		...
		{
			Byte[] dataBuffer = ReadEmbeddedFile(resourceName);
		...
		public DebugString(GraphicsContext graphics, Texture2D texture, int charaWidth, int charaHeight)
		{
			...
			if( texture != null)
				...
				this.texture = CreateTextureFromEmbeddedFile("kabuto.resources.DebugFont.png");
		...
		public void Render()
		{
			var isDepthTestEnable = graphics.IsEnabled(EnableMode.DepthTest);
		...
		{
			Byte[] dataBuffer = ReadEmbeddedFile("kabuto.shaders.SimpleSprite.cgx");
